...
public class SpeakerFlowSegmentRequestBuilder implements FlowCommandBuilder {
        ...
        for (PathSegment segment : path.getSegments()) {
            if (lastSegment == null) {
                if (doIngress) {
                    ...
                    if (ingressLoopRuleRequired(flow, ingressSide, path.isProtected())) {
                        requests.add(makeIngressLoopRequest(context, path, encapsulation, ingressSide, segment,
                                egressSide));
                    }
                }
            } else {
    ...
    private boolean transitLoopRuleRequired(Flow flow, FlowSideAdapter flowSideAdapter, boolean isProtected) {
        return flow.isLooped() && flowSideAdapter.getEndpoint().getSwitchId().equals(flow.getLoopSwitchId())
                && !isProtected;
    }

    private FlowSegmentRequestFactory makeTransitLoopRequest(
            CommandContext context, FlowPath path, FlowTransitEncapsulation encapsulation,
            PathSegment segment, FlowSideAdapter egress) {
        UUID commandId = commandIdGenerator.generate();
        MessageContext messageContext = new MessageContext(commandId.toString(), context.getCorrelationId());
        Cookie cookie = path.getCookie().toBuilder().looped(true).build();
        return TransitFlowLoopSegmentRequestFactory.builder()
                .messageContext(messageContext)
                .switchId(segment.getDestSwitch().getSwitchId())
                .metadata(makeMetadata(path.getFlow().getFlowId(), cookie, segment.isDestWithMultiTable()))
                .port(segment.getDestPort())
                .encapsulation(encapsulation)
                .build();
    }
